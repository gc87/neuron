...
int neu_json_encode_read_resp(void *json_object, void *param)
{
    ...
    for (int i = 0; i < resp->n_tag; i++) {
        ...
        void *extend_object = neu_json_encode_new();
        for (int k = 0; k < p_tag->n_meta; k++) {
            ...
            neu_json_encode_field(extend_object, &meta_elem, 1);
        }

        tag_elems[if_precision + 2].name      = "attributes";
        tag_elems[if_precision + 2].t         = NEU_JSON_OBJECT;
        tag_elems[if_precision + 2].v.val_object = extend_object;
